...
class FastestRetailScreen extends ConsumerStatefulWidget {
  const FastestRetailScreen({super.key});

  @override
  ConsumerState<FastestRetailScreen> createState() =>
      _FastestRetailScreenState();
}

class _FastestRetailScreenState extends ConsumerState<FastestRetailScreen> {
  ...
  Future<void> _loadMore() async {
    ...
    final feedData = await ref.read(
      homeFeedProvider(
        FeedParams(
          vertical: VendorCategory.retail.apiValue,
          lat: session.savedLat,
          lng: session.savedLng,
          radiusKm: 10,
          cursor: _nextCursor,
    ...
    return Scaffold(
      ...
      body: NotificationListener<ScrollNotification>(
        ...
        child: CustomScrollView(
          slivers: [
            const AppAppBar(title: 'Quick Stores'),
            SliverToBoxAdapter(
              child: Padding(
